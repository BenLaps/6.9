...
Info PostfixOrder(Node* root, Info& min, char& ch);
int Count(Node* root);
...
int main()
{
	...
	PrintTree(root, 0);
	cout << "Postfix search: ";
	PostfixOrder(root, min, ch);
	cout << endl << min << ch << endl << endl;
...
Info PostfixOrder(Node* root, Info& min, char& ch)
{
	if (root != NULL)
	{
		PostfixOrder(root->left, min, ch);
		if (root->info <= min) {
			min = root->info;
			ch = root->ch;
		}
		PostfixOrder(root->right, min, ch);
		cout << root->info << "";
		cout << root->ch << "   ";
	}
	return ch;
